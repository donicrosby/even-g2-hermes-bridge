# plugin-session-hooks

## Purpose

Defines how the even-g2 plugin subscribes to Hermes Gateway session lifecycle hooks (`on_session_start`, `on_session_end`, `on_session_finalize`, `on_session_reset`) and translates those events into structured frames the glasses-app can render. Implements the canonical Hermes pattern: the gateway's `SessionStore` is the source of truth; adapters translate events, they don't own session state.
