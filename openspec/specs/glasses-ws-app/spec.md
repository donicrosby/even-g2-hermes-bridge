...
Defines the glasses-app client behavior on the Even Hub G2 platform: WebSocket connection lifecycle, voice capture gestures (long-press, double-tap, scroll), incremental HUD rendering for assistant deltas + tool-call status + session names, touch input mapping, and background state persistence across foreground/background transitions.
