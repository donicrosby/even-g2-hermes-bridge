...
Defines how the `bridge-server/` keeps server-side conversation history for each BYOA client (since the G2 BYOA wire protocol sends only the current utterance, not prior turns). Covers history keying, turn cap, failed-turn safety, the `/clear` command, and observability.
