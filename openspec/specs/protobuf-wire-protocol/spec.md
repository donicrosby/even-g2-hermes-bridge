...
Defines the wire-format contract for the WebSocket connection between the even-g2 plugin (Python) and the glasses-app (TypeScript): a single `.proto` schema is the source of truth, every WS message is one serialized Protobuf `Frame` with a `oneof payload`, binary PCM audio is wrapped in an `AudioData` variant, the codegen pipeline produces stubs for both languages, and thin wrappers preserve the constructor call-site names.
