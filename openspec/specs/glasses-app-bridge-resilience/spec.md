# glasses-app-bridge-resilience

## Purpose

Defines the BLE-link safety invariants every `bridge.*` call in the glasses-app must satisfy: per-call timeout, serialized execution, and the design-system constraint on container borders. Implements the canonical `glasses-ui` skill patterns for Even Hub G2 apps to keep the BLE link stable under flaky radio conditions.
