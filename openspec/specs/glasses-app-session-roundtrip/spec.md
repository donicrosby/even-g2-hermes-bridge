# glasses-app-session-roundtrip

## Purpose

Defines the glasses-app side of the session contract: how it requests the session list on connect, renders the initial `active` frame, and updates on subsequent `active`/`sessions` frames. Pairs with the `plugin-session-hooks` capability (the plugin side of the contract).
