...
Defines the HTTP contract the `bridge-server/` exposes for the Even Hub "Add Agent" BYOA path: the request endpoint shape, bearer-token auth, OpenAI chat-completion response format, gzip support, health check, system-trust-store TLS, and the removal of the legacy WebSocket audio protocol.
