...
Defines the Hermes platform plugin contract for the even-g2 adapter: register() entry point via ctx.register_platform, BasePlatformAdapter interface implementation (connect/disconnect/send_message/edit_message/get_chat_info), pre/post tool-call hooks emitting tool.start/tool.end frames, env-driven configuration via BridgeConfig.from_env, and QR code generation for configuration bootstrap.
