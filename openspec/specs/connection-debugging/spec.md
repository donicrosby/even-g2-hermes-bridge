...
Defines the structured-logging and tooling requirements for diagnosing bridge connection issues: every frame is logged with structured fields, every connection lifecycle event carries a reason, a CLI debug client can reproduce failures without the physical glasses stack, and no error path is silently swallowed.
